...
const authMiddlewaker = (req, res, next) => {
     ...
     if(email===data.email && password===data.password){
      const token = jwt.sign({ email: data.email, role: "operator" }, JWT_SECRET, { expiresIn: '1h' });
      res.status(200).json({
      ...
      const token = jwt.sign({ email: email, role: "verifier" }, JWT_SECRET, { expiresIn: '1h' });
      res.status(200).json({
        message: "Login successful!",
        token,
      });
     }else if(email==="approver@gmail.com" && password==="12345678"){
      const token = jwt.sign({ email: email, role: "approver" }, JWT_SECRET, { expiresIn: '1h' });
      res.status(200).json({
        message: "Login successful!",
        token,
