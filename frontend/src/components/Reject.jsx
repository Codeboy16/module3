...
const TotalReject = ({user})=>{
  return(
    ...
    <button 
          className="bg-gradient-to-r from-red-400 via-red-500 to-red-600 text-white font-semibold py-2 px-12 rounded-lg shadow-lg hover:bg-red-700  Rounded w-full"
...
const Reject = () => {
  ...
  return (
    <>
    <h1 className='text-center py-3 bg-red-500 rounded-2xl text-white'>Total Rejected Form</h1>
    <div className='flex flex-wrap justify-center'>
    {
      user.map((userData)=>{
        return(
        <TotalReject key={userData.id} user={userData}/>
        )
      })
    }
    </div>
